...
mountPath is the canonical /app/secrets/<name>; the image's provisionValuesYaml.py reads
it via the relative path "./app/secrets/<name>", so the Job runs with workingDir "/"
(see job.yaml) to make that resolve here. subPath projects just the one key as that file.
*/}}
